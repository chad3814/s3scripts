...
SIGNATURE=`echo -ne "GET\n\n\n\nx-amz-date:$DATE\n/$BUCKET/?uploads" | openssl dgst -binary -sha1 -hmac "$AWSSEC" | openssl enc -e -a`
...
curl -H "$DATE_HEADER" -H "$AUTH_HEADER" -H "$HOST_HEADER" -s "http://s3.amazonaws.com/?uploads$MARKER" | bash -c '
ALL=$4
